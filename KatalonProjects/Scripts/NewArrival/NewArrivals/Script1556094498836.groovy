...
for (int timeRow = 1; timeRow <= findTestData('timezoneData').getRowNumbers(); timeRow++) {
    ...
    WebUI.callTestCase(findTestCase('NewArrival/productVerifyShort'), [('available') : GlobalVariable.BOShow], FailureHandling.CONTINUE_ON_FAILURE)

    /***********5 backoffice productWatchList************/
    WebUI.callTestCase(findTestCase('NewArrival/productWatchlist'), [('available') : GlobalVariable.BOEblast], FailureHandling.CONTINUE_ON_FAILURE)

    /***********6 backoffice eblast************/
    WebUI.callTestCase(findTestCase('NewArrival/productEblast'), [('available') : GlobalVariable.BOEblast], FailureHandling.CONTINUE_ON_FAILURE)

    /************7 addOn for personal order RMA *******/
    WebUI.callTestCase(findTestCase('NewArrival/addOnOrderPersonal'), [('addOnStyle') : ''], FailureHandling.CONTINUE_ON_FAILURE)
    /************8 addOn for retail order RMA *******/
    WebUI.callTestCase(findTestCase('NewArrival/addOnOrderRetail'), [('addOnStyle') : ''], FailureHandling.CONTINUE_ON_FAILURE)
