...
/***************getting DTY Not eligible date: Three days after today*************/
...
c.add(Calendar.DATE, 3) //Three days after today
...
//show DTY eleigiblity date:
c.add(Calendar.DATE, -2) 
Date showEligDate = c.getTime()
String vsyearElig = dfYYYY.format(showEligDate)
String vsmonthElig = dfMM.format(showEligDate)
String vsdayElig = dfDD.format(showEligDate)



println UtilityMethods.concat("from DTY: ", vsday, " ", vsmonth, " ", vsyear)
/***************end getting DTY not eligible date*********/
/****************************************************/
...
String DTYMsg = UtilityMethods.concat("This order will become eligible for \"Direct to You\" shipping after ", vsmonthElig, "/", vsdayElig, ".")
...
boolean hasAtributeDisabled= WebUI.verifyElementHasAttribute(findTestObject('Object Repository/Page_cabi Shipping/check_as_dty_order'), 'disabled', 
...
List<WebElement> span_place_dty_order_greys = WebUiCommonHelper.findWebElements(findTestObject('Object Repository/Page_cabi Shipping/span_place_dty_order_grey'),
	20)
...
for (WebElement we : paymentOptions) {
    ...
    if ((option.matches('.*[^\\d]\\d{4}$') || option.equalsIgnoreCase('Gift Card')) || option.equalsIgnoreCase('Add New Credit Card') || option.equalsIgnoreCase('Cash') || option.equalsIgnoreCase('Check')) {
        validPayOption = true
    }
}
